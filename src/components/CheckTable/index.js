...
export default class CheckTable extends Component {
    ...
    //         checkedFlag = true;  
    //     } 
    //     this.onCheckBox(checkedFlag, index);
    // }
    onCheckAll = bool => {
        let checkedArray= [];
        let checkedList= [];
	...
	render() {
        let { checkTableClass, columns, data, param, ...others }= this.props;
        let newColumns= this.multiSelect(columns);
        
        return <Table className={checkTableClass} columns={newColumns} data={data} {...others}/>
	}
}